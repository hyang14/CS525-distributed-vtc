class VTCReqQueue(ReqQueue):
    ...
    def generate_new_batch(self, current_batch: Batch):
        if current_batch and len(current_batch.reqs) >= self.running_max_req_size:
            return None

        can_run_list = []
        new_batch_total_tokens = 0
        active_served = dict(self.served)
        abort_list = []
        ...
        while active_served:
            ...
            if self._can_add_new_req(req, new_batch_total_tokens, len(can_run_list)):
                ...
                self.user_req_list[uid].popleft()
                # update served tokens
                cost = req.input_len * self.input_price
                self.served[uid] += cost
    ...
    def update_counter(self, current_batch: Batch):
        for req in current_batch.reqs:
            uid = req.user_id
            self.served[uid] += self.output_price  # add output-side cost
